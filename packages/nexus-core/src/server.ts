...
app.post('/task', async (req: Request, res: Response) => {
  try {
    ...
    if (!taskVector || !projectRootPath) {
      return res.status(400).json({ error: 'Missing taskVector or projectRootPath' });
    }
    ...
    console.error("[Nexus Server] A fatal error occurred during task execution:", error);
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    res.status(500).json({ error: "Nexus Core failed to execute task.", details: errorMessage });
  }
});

app.listen(port, () => {
  console.log(`[Nexus Server] Synapse Weaver Nexus Core is online and listening on http://localhost:${port}`);
});
